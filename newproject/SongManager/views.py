...
from newproject.SongManager.models import Composer, FileType
...
def JSONFileTypeListView(request):
    ...
    response = HttpResponse(simplejson.dumps(filetype_list,  ensure_ascii=False))
    return response
...
def JSONComposerListView(request):
    ...
    response = HttpResponse(simplejson.dumps(composer_list,  ensure_ascii=False))
    return response
